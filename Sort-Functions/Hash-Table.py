...
# The second column on hash table represents key.
# The first column on hash table represents data from the number of recursive calls to find the LCS.
...
# The following function evaluates how well-spread the hash function wis and count the number of collision for each-
# index of the hash table built from 1M data file.
...
for x in range(0, 10000):
    collision.append(len(H.hashmap[x]))

for v in collision:
    print("The total collision for the index:", v)

# The following function was compiled from the source referenced below:
# https://stackoverflow.com/questions/35150580/how-to-draw-bar-chart-using-plotly-offline-mode-in-python
# The following function displays the number of collisions across all indexes per bucket.
# It creates an offline bar graph to show the relationship between hashed indexes.
...
trace1 = Bar(x= xAxis,y= yAxis, width = 6, marker_color='fuchsia')
...
plotly.offline.iplot({
                        ...
                        "layout": Layout(title="<b>LCS Hash Uniformity</b>",xaxis= dict(
                                                        title= '<b>Hash keys</b>',
                                                        zeroline= False,
