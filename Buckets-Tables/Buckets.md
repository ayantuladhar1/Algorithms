...
```
    ...
    This program reads the 1M integers and splits into 100 lists with 10000 integers.
    Radix sorts 50 sub-lists with counting sort.
    Bucket sorts remaining 50 sub-lists.
    Finally merges the 100 sorted sub-lists into one sorted list
    If error occurs please check the path and make sure there are no errors.
    An example of path = C:\Users\Ayan_\PycharmProjects\merge\
    where all the sorted and splitted txt files are stored under 'Merge' folder.
    The sorting has began
    File Opening in progress
    The sorting has completed. Please check your path folder for sorted txt files.
